...
export const sanctionLoan = async (
    ...
    try {
        ...
        // FIX: only APPLIED loans can be sanctioned
...
export const rejectLoan = async (
    ...
    try {
        ...
        // FIX: only APPLIED or SANCTIONED loans can be rejected
        if (![LoanStatus.APPLIED, LoanStatus.SANCTIONED].includes(loan.status)) {
            return res.status(400).json({
                message: `Cannot reject a loan with status: ${loan.status}. Only APPLIED or SANCTIONED loans can be rejected.`
            });
        }
...
export const disburseLoan = async (
    ...
    try {
        ...
        // FIX: only SANCTIONED loans can be disbursed
...
// FIX: added try/catch — missing before, any DB error would crash the server
